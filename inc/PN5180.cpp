...
uint8_t * PN5180::readData(int len) {
  ...
  if (len < 0 || len > 625) {
    Serial.println(F("*** FATAL: Reading more than 625 bytes is not supported!"));
    PN5180DEBUG_EXIT;
    return 0L;
  }
  ...
  if (len <=16) {
    ...
    if (!readBufferDynamic508) {
       readBufferDynamic508 = (uint8_t *) malloc(625);
       if (!readBufferDynamic508) {
        PN5180DEBUG(F("Cannot allocate the read buffer of 625 Bytes!"));
        PN5180DEBUG_EXIT;
        return 0;
       }
    }
